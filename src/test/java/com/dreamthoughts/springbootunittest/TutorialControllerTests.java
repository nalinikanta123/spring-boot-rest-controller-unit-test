package com.dreamthoughts.springbootunittest;
...
import com.dreamthoughts.springbootunittest.controller.TutorialController;
import com.dreamthoughts.springbootunittest.model.Tutorial;
import com.dreamthoughts.springbootunittest.repository.TutorialRepository;
...
public class TutorialControllerTests {
	...
	void shouldReturnNoContentWhenFilter() throws Exception {
		...
		when(tutorialRepository.findByTitleContaining(title)).thenReturn(tutorials);
		mockMvc.perform(get("/api/tutorials").params(paramsMap)).andExpect(status().isNoContent()).andDo(print());
	}
	...
	void shouldReturnNotFoundUpdateTutorial() throws Exception {
		...
		mockMvc.perform(put("/api/tutorials/{id}", id).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(updatedtutorial))).andExpect(status().isNotFound())
				.andDo(print());
	}
	...
	void shouldDeleteTutorial() throws Exception {
		...
		mockMvc.perform(delete("/api/tutorials/{id}", id)).andExpect(status().isNoContent()).andDo(print());
	}

	@Test
	void shouldDeleteAllTutorials() throws Exception {
		doNothing().when(tutorialRepository).deleteAll();
		mockMvc.perform(delete("/api/tutorials")).andExpect(status().isNoContent()).andDo(print());
	}
}
